    ...
    (function() {
        ...
        function decodeConfig(config) {
            if (config && typeof config.termsText === 'string') {
                try {
                    ...
                    console.error("[Loader] Failed to decode termsText base64 string: " + e.message);
                }
            }
            return config;
        }
        ...
        if (configText) {
            try {
                ...
                console.error("[Loader] Failed to parse embedded AnkiFXConfig JSON. Falling back to _afx_defaults.json. Error: " + e.message);
            }
        }
                ...
                if (xhr.readyState === 4) {
                    if (xhr.status === 200 || xhr.status === 0) {
                        try {
                            ...
                            console.error("[Loader] Failed to parse fallback _afx_defaults.json: " + err.message);
                        }
                    } else {
                        console.error("[Loader] Failed to load fallback _afx_defaults.json. Status: " + xhr.status);
                    }
                }
            };
            xhr.send();
...
<script src="_ankifx.js" onerror="console.warn('[Loader] Local engine backup not found in collection.media.')"></script>

<!-- Load the latest remote engine CDN (parsed sequentially, overrides local global if online) -->
<script id="ankifx-engine-script" src="https://cdn.jsdelivr.net/gh/robkipa/ankifx@latest/build/_ankifx.js" onerror="console.warn('[Loader] CDN failed to load, using local engine.')"></script>
...
<script>
    (function() {
        ...
        if (remoteScript) {
            ...
            } else if (window.AnkiFX && window.AnkiFX.initialized) {
                window.AnkiFX_Remote_Status = "loaded";
                afxLog("Engine already running, remote check bypassed.", "success");
            } else {
...
<script>
    ...
    function triggerAnkiFX(attempts = 0) {
        ...
        if (hasAnkiFX && hasRun && hasConfig) {
            ...
            const isWaitingForRemote = !isAlreadyInitialized && (remoteStatus === "pending") && (attempts < 16);
            ...
            if (!ankiFXInitialized) {
                try {
                    ...
                    console.error("[Loader] Start Error: " + e.message);
                }
            }
            ...
            if (!contentInitialized) {
                try {
                    ...
                    console.error("[Loader] Card Content Run Error: " + e.message);
                }
            }
            ...
            if (hasRun && !contentInitialized) {
                try {
                    ...
                    console.error("[Loader] Fallback Card Content Run Error: " + runErr.message);
                }
            }
        }
    }
